...
class UsersViewSet(UserViewSet):
    ...
    def subscribe(self, request, id=None):
        ...
        if Subscribe.objects.filter(user=user, author=author).exists():
            return Response(
                {"errors": "Уже подписан."}, status=HTTPStatus.BAD_REQUEST
            )
    ...
    def del_subscribe(self, request, id=None):
        ...
        if not follow.exists():
            return Response(
                {"errors": "Уже отписались."}, status=HTTPStatus.BAD_REQUEST
            )
        follow.delete()
...
class RecipesViewSet(viewsets.ModelViewSet, mixins.ListModelMixin):
